...
auto dsigmoid(const double& y) -> double {
...
NeuralNetwork::NeuralNetwork(const std::uint32_t& input,
  ...
  : input_nodes(input), hidden_nodes(hidden), output_nodes(output),
    ...
    this->matrices[0] = Matrix(hidden, input);
    this->matrices[1] = Matrix(output, hidden);
    this->matrices[2] = Matrix(hidden, 1);
    this->matrices[3] = Matrix(output, 1);
...
auto NeuralNetwork::dumps() const noexcept -> std::string {
    ...
    _str += std::string(",\"bias_h\":") + this->matrices[2].dumps()
          + std::string(",\"bias_o\":") + this->matrices[3].dumps()
          + std::string(",\"input_nodes\":") + std::to_string(this->input_nodes)
          + std::string(",\"hidden_nodes\":") + std::to_string(this->hidden_nodes)
          + std::string(",\"output_nodes\":") + std::to_string(this->output_nodes)
          + std::string(",\"weights_ih\":") + this->matrices[0].dumps()
          + std::string(",\"weights_ho\":") + this->matrices[1].dumps()
...
auto NeuralNetwork::copy(const NeuralNetwork& nn) noexcept -> NeuralNetwork {
    ...
    tmp.matrices[0] = Matrix(hidden, input);
    tmp.matrices[1] = Matrix(output, hidden);
    tmp.matrices[2] = Matrix(hidden, 1);
    tmp.matrices[3] = Matrix(output, 1);

    for (int i = 0; i < static_cast<int>(nn.matrices.size()); ++i) {
