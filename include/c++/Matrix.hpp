...
class Matrix {
 public:
    ...
    auto operator+=(const float_t&) -> Matrix&;
    auto operator*=(const Matrix&) -> Matrix&;
    auto operator*=(const float_t&) -> Matrix&;
    ...
    // Functions
    auto begin() const noexcept -> const float_t * {
        return &this->data[0];
    }
    auto end() const noexcept -> const float_t * {
        return this->iterator;
    }
    auto toArray() const noexcept -> float_t*;
    void randomize();
    void map(float_t (*const &)(float_t));
    auto serialize() const noexcept -> const nlohmann::json;

    // Static functions
    static auto fromArray(const float_t* const&, const int&) -> Matrix;
